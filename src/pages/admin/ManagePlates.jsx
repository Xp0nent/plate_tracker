...
const getStatusColor = (status) => {
  if (status?.includes('RELEASED')) return COLORS.danger;
  if (status?.includes('AVAILABLE')) return COLORS.success;
...
export default function ManagePlates() {
  ...
  const columns = useMemo(() => [
    ...
    { field: 'status', headerName: 'STATUS', width: 180, renderCell: (p) => (
        <Chip 
          label={p.value?.toUpperCase()} 
          size="small" 
          variant="outlined" 
          sx={{ color: getStatusColor(p.value), borderColor: getStatusColor(p.value), fontWeight: 800, fontSize: '0.65rem' }} 
        />
    )},
